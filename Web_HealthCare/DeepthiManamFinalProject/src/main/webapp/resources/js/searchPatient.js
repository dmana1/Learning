$(function() {   
 $("#searchById").submit(function( event ) {
    	$("#error").html(" ");    	
        var mrnExp=/^\d+$/;
        var isValid=true; 

        if (!$("#patientId").val().match(mrnExp)) {
 ...
 $("#quickSearchResults").submit(function( event ) {
     ...
     if(!$("#ssn").val() && !$("#firstName").val() && !$("#lastName").val() && !$("#phoneNumber").val()){
    	 $("#error").append("<p>Please enter atleast one criteria to search</p>");
       	 isValid=false; 
     }
